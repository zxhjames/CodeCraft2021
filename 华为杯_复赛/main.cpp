    ...
    void readDataFromFile() {
        ...
        for (int i = 0; i < t; ++i) {
            ...
            while (r--) {
                ...
                dayOpList[i].push_back((OP) { a, id, name });
            }
        }
    }

    void migrate() {
        int migrationNum = vms.size() * 5 / 1000;
        if (migrationNum == 0) {
            return;
        }
...
//
    Manager manager1;
    manager1.setFile("/Users/niyijie/Desktop/HWCup/HWCup/training_1.txt");
    manager1.setPrint(0);
    manager1.readDataFromFile();
    manager1.run();
    manager1.printResult();

    Manager manager2;
    manager2.setFile("/Users/niyijie/Desktop/HWCup/HWCup/training_2.txt");
    manager2.setPrint(0);
    manager2.readDataFromFile();
    manager2.run();
